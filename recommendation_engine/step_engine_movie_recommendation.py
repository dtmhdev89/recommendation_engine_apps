...
class DataPreprocess():
    def perform(self, dataset_url=None):
        if dataset_url:
...
class RecommendationModel():
    def __init__(self, data):
        self.data = data
        self.reader = self.build_reader()
        self.model = self.prepare_model()
    
    def build_reader(self):
        return Reader(
            rating_scale=(self.data["score"].min(), self.data["score"].max())
        )
    
    def build_dataset(self):
        dataset = Dataset.load_from_df(
            self.data[['orig_lang', 'orig_title', 'score']],
            self.reader
        )
...
def get_recommendations(model, data, selected_movies, genre):
...
def main():
    data = DataPreprocess().perform()
    model = RecommendationModel(data=data).model

    def build_gradio_interface(model, data):
        def gr_get_recommendation(selected_movies, genre):
            return get_recommendations(model, data, selected_movies, genre)

        def gr_get_movies_by_genre(genre):
            return get_movies_by_genre(data, genre)

        def movie_recommeder_ui(genre, selected_movies):
            recommendations = gr_get_recommendation(selected_movies, genre)
        ...
        with gr.Blocks() as demo:
            ...
            def update_movies(genre):
                return gr.update(choices=gr_get_movies_by_genre(genre), value=[])
